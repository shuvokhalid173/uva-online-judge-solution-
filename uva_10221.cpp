/// be careful about angle it may be greater than 180
/// which is invalid in this problem thats why we have to substract it from 360;
/******* header files *******/
#include <algorithm>
#include <bitset>
#include <cctype>
...
int main (){
	double radius , angle;
	char unit[4];
	while (scanf ("%lf%lf%s" , &radius , &angle , &unit) == 3){
		...
		double S = H + radius;
		double arc_length = S * theta;
		double org_length = sqrt ((2 * S * S * (1 - cos (theta))));
		printf ("%.6lf %.6lf\n" , arc_length , org_length);
	}
}
